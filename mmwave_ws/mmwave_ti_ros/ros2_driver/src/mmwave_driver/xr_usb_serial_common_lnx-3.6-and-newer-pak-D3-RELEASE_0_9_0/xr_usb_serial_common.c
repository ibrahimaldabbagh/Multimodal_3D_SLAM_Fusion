...
static ssize_t xr_usb_serial_tty_write(struct tty_struct *tty, const u8 *buf, size_t count)
{
	struct xr_usb_serial *xr_usb_serial = tty->driver_data;
	int stat;
	unsigned long flags;
...
static void xr_usb_serial_tty_set_termios(struct tty_struct *tty, const struct ktermios *termios)
{
	struct xr_usb_serial *xr_usb_serial = tty->driver_data;
...
{
	int retval;
	xr_usb_serial_tty_driver = __tty_alloc_driver(XR_USB_SERIAL_TTY_MINORS, THIS_MODULE, (unsigned long)0);
