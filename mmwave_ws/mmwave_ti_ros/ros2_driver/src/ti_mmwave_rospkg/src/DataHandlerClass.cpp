    ...
    while (rclcpp::ok())
    {
        ...
        case READ_OBJ_STRUCT:
        {
            ...
            while (i < mmwData.numObjOut)
            {
                ...
                { // SDK version is 3.x+
                    ...
                    // RCLCPP_INFO(rclcpp::get_logger("rclcpp"),"VELOCITY = %f", mmwData.newObjOut.velocity);
                    //  Map mmWave sensor coordinates to ROS coordinate system
        ...
        case READ_SIDE_INFO:
        {
            ...
            if (mmwData.numObjOut > 0)
            {
                for (i = 0; i < mmwData.numObjOut; i++)
                {
                    ...
                    /*RCLCPP_INFO(rclcpp::get_logger("rclcpp"),"RSCAN Intensity : %f", RScan->points[1].intensity);
                    RCLCPP_INFO(rclcpp::get_logger("rclcpp"),"RSCAN Intensity Nr.2 : %f", RScan->points[2].intensity);
                    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Noise Value Nr2 : %d ", mmwData.sideInfo.noise);*/
                }
            }
        ...
        case CHECK_TLV_TYPE:
        {
            ...
            if (tlvCount++ >= mmwData.header.numTLVs) // Done parsing all received TLV sections
            {
                ...
                // RCLCPP_INFO(rclcpp::get_logger("rclcpp"),"RSCAN Velocity: %f", RScan->points[1].velocity);
                // // RCLCPP_INFO(rclcpp::get_logger("rclcpp"),"RSCAN X : %f", RScan->points[2].x);
                ...
                if (first_msg == true)
                {
                    ...
                    time_pc = nodeHandle->now();

                    time_pc = time_pc - std::chrono::nanoseconds(70000000); // Ausgabezeit UART für erste Nachricht wegrechnen;
                    time_pc = time_pc + std::chrono::nanoseconds(15000000); // Halbe Gesamtemessdauer von FMCW-Messung

                    first_msg = false;
                }

                pc2_msg_->header.stamp = time_pc;
                pc2_msg_->header.frame_id = frameID;
                radar_scan_pcl_pub->publish(*pc2_msg_);
                time_pc = time_pc + std::chrono::nanoseconds(70000000);
                sorterState = SWAP_BUFFERS;
            }
